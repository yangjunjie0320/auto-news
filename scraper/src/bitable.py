...
class BitableSyncer:
    ...
    def _batch_create(self, batch: list[tuple[str, dict]]) -> None:
        records = [
            AppTableRecord.builder().fields(self._to_fields(mid, rec)).build() for mid, rec in batch
        ]
        request = (
        ...
        if not response.success():
            raise BitableError(f"batch create failed: code={response.code} msg={response.msg}")
        ...
        if created_records is None or len(created_records) != len(batch):
            ...
            raise BitableError(f"batch create returned {actual} record(s), expected {len(batch)}")

    def _remote_has_mid(self, mid: str) -> bool:
        condition = Condition.builder().field_name("mid").operator("is").value([mid]).build()
    ...
    def _ensure_fields(self) -> None:
        ...
        for field_name, field_type in _FIELDS:
            ...
            if not response.success():
                raise BitableError(
                    f"field create failed: {field_name}: code={response.code} msg={response.msg}"
                )
    ...
    def _list_fields(self) -> set[str]:
        ...
        while True:
            ...
            if not response.success():
                raise BitableError(f"field list failed: code={response.code} msg={response.msg}")
    ...
    def _resolve_app_token(self) -> str:
        ...
        if not node or node.obj_type != "bitable" or not node.obj_token:
            raise BitableError(f"wiki 节点不是多维表格: obj_type={getattr(node, 'obj_type', None)}")
    ...
    def _list_tables(self) -> list[tuple[str, str]]:
        ...
        while True:
            ...
            if not response.success():
                raise BitableError(f"table list failed: code={response.code} msg={response.msg}")
            for item in response.data.items or []:
    ...
    def _create_table(self, name: str) -> str:
        ...
        if not response.success():
            raise BitableError(f"table create failed: code={response.code} msg={response.msg}")
