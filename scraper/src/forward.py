...
class ForwardStore:
    ...
    def due_delivery(self, *, now: dt.datetime | None = None) -> list[tuple[str, dict[str, Any]]]:
    ...
    def due_cards(self, *, now: dt.datetime | None = None) -> list[tuple[str, dict[str, Any]]]:
        now = (now or _utcnow()).astimezone(dt.UTC)
        with self._lock:
    ...
    def pending_archive(
        ...
        with self._lock:
            ...
            for mid, record in self._forwards.items():
                if record["delivery_status"] != "forwarded" or record["card_status"] != "patched":
                    continue
    ...
    def mark_delivery_forwarded(self, mid: str, *, now: dt.datetime | None = None) -> None:
        now = now or _utcnow()
        with self._lock:
    ...
    def mark_archive_synced(self, mids: list[str], *, now: dt.datetime | None = None) -> None:
        if not mids:
            return
        now = now or _utcnow()
        with self._lock:
