...
from src.image_uploader import ImageFetchError, _download_image, _headers_for, _safe_url_for_log


def test_headers_for_adds_weibo_referer_only_for_sinaimg():
    # 微博图床防盗链要求 weibo Referer；易车/汽车之家图床带了反而被拒
    assert _headers_for("https://wx1.sinaimg.cn/mw2000/a.jpg")["Referer"] == "https://weibo.com/"
    assert _headers_for("https://n.sinaimg.cn/a.jpg")["Referer"] == "https://weibo.com/"
    assert "Referer" not in _headers_for("https://img.bitautoimg.com/a.jpg")
    assert "Referer" not in _headers_for("https://car2.autoimg.cn/a.jpg")
    assert "Referer" not in _headers_for("https://evil.com/sinaimg.cn/a.jpg")
...
@pytest.mark.parametrize(
    "host",
    ["image.bitautoimg.com", "img1.bitauto.com", "www2.autoimg.cn"],
)
async def test_download_image_allows_all_configured_source_domains(host):
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, headers={"Content-Type": "image/jpeg"}, content=b"img")

    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        assert await _download_image(f"https://{host}/a.jpg", client) == b"img"
...
async def test_download_image_rejects_redirect_to_untrusted_host():
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        with pytest.raises(ImageFetchError, match="approved source image domain"):
